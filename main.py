...
def save_qr_code(qr_image, value):
    file_path = filedialog.asksaveasfilename(
        defaultextension=".png",
        filetypes=[("PNG files", "*.png"), ("All files", "*.*")],
        initialfile=f"{value}.png",
    )
    if file_path:
...
def generate_qr_codes():
    ...
    for value in values:
        ...
        save_button = tk.Button(
            qr_code_container,
            text="Save",
            command=lambda qr_image=qr_image, value=value: save_qr_code(
                qr_image, value
            ),
        )
...
generate_button = tk.Button(
    root, text="Generate QR Codes", command=on_generate_button_click
)
